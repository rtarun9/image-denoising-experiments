...
class HformerSAModel(torch.nn.Module):
    def __init__(self, num_channels,width=64, height=64, device='cuda'):
        super(HformerSAModel, self).__init__()
        ...
        self.downsampling_layer_2 = torch.nn.Conv2d(num_channels * 2, num_channels * 4, kernel_size=(2,2), stride=2).to(device)
        
        self.upsampling_layer_1 = torch.nn.ConvTranspose2d(num_channels * 4, num_channels * 2, kernel_size=(2, 2), stride=2).to(device)
        self.upsampling_layer_2 = torch.nn.ConvTranspose2d(num_channels * 2, num_channels, kernel_size=(2, 2), stride=2).to(device)
        ...
        self.hformer_block_2 = HformerBlock(maxpool_kernel_size=(2,2), attention_scaling_factor=1, num_channels=num_channels * 4, width=width // 4, height=height // 4).to(device)
        self.hformer_block_3 = HformerBlock(maxpool_kernel_size=(2,2), attention_scaling_factor=1, num_channels=num_channels * 2, width=width // 2, height=height // 2).to(device)
