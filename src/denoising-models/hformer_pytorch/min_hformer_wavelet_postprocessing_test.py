...
def visualize_patches(patches, noisy_images,  ground_truth_images, num_images):
    ...
    for i in range(num_images):
        ...
        print(compute_PSNR(_clean_image, reconstruct_image, 240.0 + 160.0),
              compute_PSNR(_clean_image, _noisy_image, 240.0 + 160.0), 
              calculate_ssim(_clean_image, _noisy_image),
              calculate_ssim(_clean_image, reconstruct_image))
...
def evaluate_model_wavelet_swap(model, history, name, psnr_or_accuracy):
    ...
    with torch.no_grad():    
        for i, data in enumerate(training_dataloader):
            ...
            clean_patches = clean.detach().cpu()
            noisy_patches = noisy.detach().cpu()

            visualize_patches(prediction_patches, noisy_patches, clean_patches,2)
