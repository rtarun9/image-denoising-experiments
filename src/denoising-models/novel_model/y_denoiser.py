...
class YModel(torch.nn.Module):
    ...
    def forward(self, images):
        ...
        y1= self.y_block_1(cb1)

        down1 = self.downsampling_layer_1(y1)

        cb2  = self.conv_block_2(down1)
        y2 =self.y_block_2(cb2)

        down2 = self.downsampling_layer_2(y2)
        ...
        y4 = self.y_block_4(up1) + y2
        cb5 = self.conv_block_5(y4) + cb2

        up2 = self.upsampling_layer_2(cb5)

        y5 = self.y_block_5(up2) + y1
        cb6 = self.conv_block_6(y5) + cb1
