...
from tensorflow.compat.v1 import layers as slim
import numpy as np
from network import network

from data_importer import load_training_images

training_images_x, training_images_y = load_training_images('../../../../../Dataset/LowDoseCTGrandChallenge/Training_Image_Data', load_limited_images=True, num_images_to_load=4)
...
for epoch in range(lastepoch, 4001):
    ...
    for ind in np.random.permutation(len(training_images_x)):
        ...
        train_batch = training_images_x[ind]
        gt_batch = training_images_y[ind]
    ...
    epoch_summary = tf.v1.Summary(value=[tf.Summary.Value(tag='epoch_loss', simple_value=epoch_loss)])
